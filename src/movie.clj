...
(defn update-last-watched [path]
  (let [movie-info (@movie-infos path)
        time (. System currentTimeMillis)]
    (cond (nil? movie-info)
          (common/ref-add movie-infos {path (struct MovieInfo path time nil nil nil nil nil nil)})
          :else
          (common/ref-upd-nil movie-infos (assoc @movie-infos path (assoc movie-info :last-watched time))))))
...
(defn get-movie-info-file [file]
  (let [f (. file getName)
    ...
    {(. file getPath) (struct MovieInfo (. file getPath) nil (:year imdb-info) (:rating imdb-info) (:url imdb-info) (:img rotten-info) (:rating rotten-info) (:url rotten-info))}))
...
(when (nil? @movie-infos)
  (common/ref-upd-nil movie-infos {}))
